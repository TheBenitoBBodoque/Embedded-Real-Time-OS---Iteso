...
#define SCI115200bps    (UINT16)((UINT32)(mcal_mcu_gu32BusFrequency) / (UINT32)(1843200u))/* (115200 * 16) */ 
#define SCI57600bps     (UINT16)((UINT32)(mcal_mcu_gu32BusFrequency) / (UINT32)(921600u)) /* (57600 * 16) */   
#define SCI38400bps     (UINT16)((UINT32)(mcal_mcu_gu32BusFrequency) / (UINT32)(614400u)) /* (38400 * 16) */
#define SCI19200bps     (UINT16)((UINT32)(mcal_mcu_gu32BusFrequency) / (UINT32)(307200u)) /* (19200 * 16) */
#define SCI9600bps      (UINT16)((UINT32)(mcal_mcu_gu32BusFrequency) / (UINT32)(153600u)) /* (9600 * 16)  */
#define SCI4800bps      (UINT16)((UINT32)(mcal_mcu_gu32BusFrequency) / (UINT32)(76800u))  /* (4800 * 16)  */


/*-- Register definitions ----------------------------------------------------*/

#define SCI_MAIN_NODE_OFS                   (UINT8)8u
...
#define SCI0_WRITE_BD(NodeIndex, Value)     (*((&SCI0BD)  + (SCI_MAIN_NODE_OFS * NodeIndex)) = (UINT16)Value)
/* Extra Macro to configure SCI1's Baudrate to fix addressing Bug */ 
#define SCI1_WRITE_BD(NodeIndex, Value)     (*((&SCI1BD)  + (SCI_MAIN_NODE_OFS * (NodeIndex-1))) = (UINT16)Value)

#define SCI_WRITE_CR1(NodeIndex, Value)     (*((&SCI0CR1) + (SCI_MAIN_NODE_OFS * NodeIndex)) = (UINT8) Value)
#define SCI1_WRITE_CR1(NodeIndex, Value)    (*((&SCI1CR1) + (SCI_MAIN_NODE_OFS * (NodeIndex-1))) = (UINT8) Value)

#define SCI_WRITE_CR2(NodeIndex, Value)     (*((&SCI0CR2) + (SCI_MAIN_NODE_OFS * NodeIndex)) = (UINT8) Value)
#define SCI1_WRITE_CR2(NodeIndex, Value)    (*((&SCI1CR2) + (SCI_MAIN_NODE_OFS * (NodeIndex-1))) = (UINT8) Value)
...
void SendToggleBreak(enum tSCI_Channel IDChannel);

void SetBreakLenght(enum tSCI_Channel IDChannel, enum tSCI_BrkLenght enLenght);

void BrkDetectFeature(enum tSCI_Channel IDChannel, UINT8 u8State);

UINT8 GetBreakFlag(enum tSCI_Channel IDChannel);

void ClearBreakFlag(enum tSCI_Channel IDChannel);
