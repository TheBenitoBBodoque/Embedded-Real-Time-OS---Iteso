...
void interrupt  vfnPIT_Channel0_Isr( void  )
{
    ...
    __asm
    {
        CALL    Dispatcher                  ; (PC) Load from a fixed memory location to Register D
    }
}

void interrupt  vfnPIT_Channel1_Isr( void  )
{
    static u8 counter_100ms;
    /* Verify that Real Time Interrupt caused the interrupt */
    if( PITTF_PTF1 == 1u )
    {
      if(counter_100ms==1){
        ...
        {
            gInterrupt_PIT_flag|=0x02;//Gpt_ConfigType_initial->ptr_Pit_ChannelConfig[1].Pit_Channel_Callback();
        } 
      }else{
      counter_100ms++;
      }
...
void interrupt  vfnPIT_Channel2_Isr( void  )
{
    ...
    if( PITTF_PTF2 == 1u )
    {
      if(counter_200ms==3){
        ...
        {
            gInterrupt_PIT_flag|=0x04;//Gpt_ConfigType_initial->ptr_Pit_ChannelConfig[2].Pit_Channel_Callback();
        }
      }else{ 
        counter_200ms++;
      }
...
void interrupt  vfnPIT_Channel3_Isr( void  )
{
    ...
    {  
        ...
        {
            gInterrupt_PIT_flag|=0x08;//Gpt_ConfigType_initial->ptr_Pit_ChannelConfig[3].Pit_Channel_Callback();
        }  
    }
    /* Clear the real time interrupt flag */
    PITTF_PTF3 = 1u;
